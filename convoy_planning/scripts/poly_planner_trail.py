...
from convoy_planning.plan_utils import line_of_best_fit, get_centerline, \
    create_marker_array, create_trajectory_msg
...
class PolyPlanner(Node):
    ...
    def __init__(self):
        ...
        # ROS parameters
        veh_ns = self.declare_parameter(
            'veh_ns').get_parameter_value().string_value
        self.veh_frame = self.declare_parameter(
        ...
        course_data_topic = self.declare_parameter(
            'course_data_topic').get_parameter_value().string_value
        self.controller = self.declare_parameter(
            'controller').get_parameter_value().string_value
        self.deg = self.declare_parameter(
            'poly_deg').get_parameter_value().integer_value
        self.traj_length = self.declare_parameter(
            'trajectory_length').get_parameter_value().integer_value
        self.H = self.declare_parameter(
            'planning_horizon', 0).get_parameter_value().integer_value
        traj_topic = self.declare_parameter(
            'planner_trajectory_topic').get_parameter_value().string_value
        traj_vis_topic = self.declare_parameter(
            'planner_trajectory_vis_topic').get_parameter_value().string_value
        self.wheelbase = self.declare_parameter(
            'wheelbase').get_parameter_value().double_value
        neighbor_info_topic = self.declare_parameter('neighbor_info_topic').get_parameter_value().string_value
        ...
        # ROS subscribers
        self.create_subscription(CourseData, course_data_topic, 
                                 self.course_data_cb, 1)
    ...
    def neighbor_info_cb(self, msg):
        ...
        self.theta_rel = quat2euler([msg.relative_pose.orientation.w,
                                     msg.relative_pose.orientation.x,
                                     msg.relative_pose.orientation.y,
                                     msg.relative_pose.orientation.z])[2]
    ...
    def course_data_cb(self, msg):
        ...
        if right_pc.shape[0] > 0 and left_pc.shape[0] > 0:
            ...
            furthest_near_point = np.array([furthest_near_x, 
                                            furthest_near_y])

            center_traj_far = (traj_far + furthest_near_point) / 2.
            if self.use_sim:
        ...
        markers = create_marker_array(constr_traj_points, self.traj_color, 
                                      self.veh_frame)
