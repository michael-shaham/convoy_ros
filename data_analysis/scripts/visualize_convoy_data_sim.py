    ...
    for i in range(n_vehicles):
        ...
        if i > 0:
            ...
            for i in range(len(neighbor[0, :])):
                if i >= ma_window:
                    neighbor[1, i] = neighbor[1, i-ma_window+1:i+1].sum() / ma_window
...
if __name__ == '__main__':
    ...
    for i in range(n_vehicles):
        ax[0].plot(speed_data[i][0] - start_time, speed_data[i][1], 
                   label=f"vehicle {i}")
    ...
    for i in range(n_vehicles - 1):
        ax[1].plot(neighbor_data[i][0], neighbor_data[i][1], 
                   label=f"vehicle {i+1} to {i}")
    ax[1].plot(neighbor_data[1][0], 
    ...
    for i in range(n_vehicles - 1):
        if i == 0:
            ax[2].plot(neighbor_data[i][0] , 
                       neighbor_data[i][1] - attack_distance, 
                       label=f"vehicle {i+1} to {i}")
        else:
            ax[2].plot(neighbor_data[i][0] , neighbor_data[i][1], 
                       label=f"vehicle {i+1} to {i}")
    ax[2].plot(neighbor_data[1][0] , 
    ...
    plt.subplots_adjust(left=0.07, bottom=0.07, right=0.72, top=0.90, wspace=0.20, hspace=0.24)
