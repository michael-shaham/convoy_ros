...
if __name__ == '__main__':
    ...
    for i in range(1, n_vehicles):
        ...
        ax[0].plot(veh_err['time'].to_numpy(), veh_err['dist_err'].to_numpy(), 
                   label='veh_' + str(i))

        ax[1].plot(veh_err['time'].to_numpy(), veh_err['speed_err'].to_numpy(), 
                   label='veh_' + str(i))
    ...
    fig.subplots_adjust(.06, .08, .89, .92, .20, .11)
