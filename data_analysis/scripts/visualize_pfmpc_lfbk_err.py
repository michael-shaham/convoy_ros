...
if __name__ == '__main__':
    ...
    pfmpc_df_errors = pfmpc_df[['veh_ind', 'time', 'dist_err', 'speed_err', 
                                'ego_speed', 'opt_input']]
    lfbk_df_errors = lfbk_df[['veh_ind', 'time', 'dist_err', 'speed_err', 
                              'ego_speed', 'opt_input']]
    n_vehicles = 7
    ...
    fig.suptitle('Platoon performance comparison: 1.5-3 m/s alternating', 
                 fontsize=title_size)
    ...
    veh_inds = [i for i in range(1, n_vehicles) 
                if i in pfmpc_df_errors['veh_ind'].to_numpy()]
    ...
    for k, i in enumerate(veh_inds):
        ...
        pfmpc_dist_err_rms[i] = \
            np.sqrt(np.sum(np.square(pfmpc_dist_err)) / len(pfmpc_dist_err))
        pfmpc_speed_err_rms[i] = \
            np.sqrt(np.sum(np.square(pfmpc_speed_err)) / len(pfmpc_speed_err))

        lfbk_dist_err_rms[i] = \
            np.sqrt(np.sum(np.square(lfbk_dist_err)) / len(lfbk_dist_err))
        lfbk_speed_err_rms[i] = \
            np.sqrt(np.sum(np.square(lfbk_speed_err)) / len(lfbk_speed_err))

        ax[0,0].plot(pfmpc_veh_err['time'].to_numpy(), 
                     pfmpc_veh_err['dist_err'].to_numpy(), 
                     label=f"{k+1}",
                     linewidth=linewidth, linestyle=linestyle, alpha=alphas[k])
        ax[0,1].plot(lfbk_veh_err['time'].to_numpy(), 
    ...
    fig.subplots_adjust(.13, .16, .84, .82, .07, .09)
    plt.show()
