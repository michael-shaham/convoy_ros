...
if __name__ == '__main__':
    ...
    for i, file in enumerate(data_files):
        ...
        dt_discrete = 0.02
        time_discrete = np.arange(0., df['time'].to_numpy()[-1], dt_discrete)

        dt_cont = 0.02
        time_cont = np.arange(0., df['time'].to_numpy()[-1], dt_cont)
        ...
        for i in range(len(v_discrete) - 1):
            v_discrete[i+1] = forward_discrete(v_discrete[i], desired_speed[i], dt_discrete, tau)
        ...
        ax.plot(df['time'].to_numpy(), df['desired speed'].to_numpy(), 
                   label='desired speed')
