...
def load_data(test_name: str, n_vehicles: int):
    ...
    """
    data_path = os.path.join(os.path.expanduser('~'), 'cvy_ws', 'src', 
...
if __name__ == '__main__':
    ...
    fig.suptitle('Convoy dynamics - 3 vehicles with sensor attack on vehicle 1',
                 fontsize=title_size)
    ...
    for i in range(n_vehicles-1):
        ax[0].plot(neighbor_data[i][0] - start_times[i], neighbor_data[i][2], 
                   label=f"vehicle {i}")
    ax[0].plot(speed_data[2][0] - start_times[2], speed_data[2][1], 
               label="vehicle 2")
    ...
    for i in range(n_vehicles - 1):
        if i == 0:
            ax[1].plot(neighbor_data[i][0] - start_times[i], 
                       neighbor_data[i][1] - attack_distance, 
                       label=f"vehicle {i+1} to {i}")
        else:
            ax[1].plot(neighbor_data[i][0] - start_times[i], 
                       neighbor_data[i][1], label=f"vehicle {i+1} to {i}")
    ax[1].plot(neighbor_data[1][0] - start_times[1], 
    ...
    save_file = os.path.join(os.path.expanduser('~'), 'cvy_ws', 'src', 
                            'convoy_ros', 'data_analysis', 'scripts', 
                            'figures', test_name + '.pdf')
    plt.xlim(-2, 66) 
    plt.subplots_adjust(left=0.05, bottom=0.10, right=0.79, top=0.87, wspace=0.20, hspace=0.30)
    plt.subplot_tool()
    ...
    plt.figure()
    for i in range(n_vehicles - 1):
        plt.plot(range(len(distance_errors[i])), distance_errors[i], label=f"vehicle {i + 1}")
