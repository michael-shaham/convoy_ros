...
def plot_quad_l1_lfbk(quad_df, l1_df, lfbk_df):
    ...
    fig.suptitle('Platoon experiment example', 
                 fontsize=title_size)
    ...
    ax[1,2].legend(bbox_to_anchor=(0.99, 0.5), loc="center left", 
                   title="vehicle", title_fontsize=legend_title_size,
                   fontsize=legend_font_size)
    plt.show()
...
if __name__ == '__main__':
    ...
    quad_pfmpc_df = pd.concat([pd.read_csv(data_loc + file) for file in quad_pfmpc_files])
