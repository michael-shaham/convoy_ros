...
save_file_exp = prefix + "/data_analysis/figures/iros_2024_platoon_exp.pdf"
save_file_rms = prefix + "/data_analysis/figures/iros_2024_rms_fig.pdf"
...
def plot_quad_nn_lfbk(quad_df, nn_df, lfbk_df):
    ...
    ax[0, 0].set_title("DMPC", fontsize=subtitle_size)
    ax[0, 1].set_title("Neural network", fontsize=subtitle_size)
    ax[0, 2].set_title("Linear feedback", fontsize=subtitle_size)

    for i in veh_inds:
        quad_pfmpc_veh = quad_df[quad_df["veh_ind"] == i]
        nn_veh = nn_df[nn_df["veh_ind"] == i]
        ...
        ax[0, 1].plot(
            nn_veh["time"].to_numpy(),
            nn_veh["ego_position"].to_numpy(),
            label=f"{i}",
            zorder=max(veh_inds) - i,
            color="C{}".format(i),
            linewidth=linewidth,
...
def plot_nn_trajectory(nn_df):
    veh_inds = [i for i in range(max(nn_df["veh_ind"]) + 1)]

    fig, ax = plt.subplots(1, 2, figsize=(2.95, 1.5))
    fig.subplots_adjust(0.08, 0.26, 0.84, 0.77, 0.2, 0.2)
    fig.suptitle(
        f"Platoon trajectory: Neural network controller", fontsize=title_size, x=0.5
    )
    ...
    for a in ax.flat:
        a.grid()
        a.tick_params(axis="both", labelsize=tick_label_size)
    ax[0].set_xticks([0.0, 10.0, 20.0, 30.0])
    ax[1].set_xticks([0.0, 10.0, 20.0, 30.0])
...
def plot_mean_std_rms_error(
    ...
    fig, ax = plt.subplots(1, 2, sharey="row", figsize=(2.95, 1.5))
    fig.subplots_adjust(0.14, 0.25, 0.98, 0.77, 0.1, 0.2)
    fig.suptitle("Mean RMSE with 95% confidence intervals", fontsize=title_size, x=0.54)
    ax[0].set_title("Position error", fontsize=subtitle_size)
    ...
    for i, method in enumerate(method_names):
        ...
        veh_inds = range(2, len(dist_rms_means) + 2)
...
if __name__ == "__main__":
    ...
    nn_files = [file for file in data_files if "nn" in file]
    ...
    lfbk_plot_ind = 0
    quad_pfmpc_plot_ind = 0
    nn_plot_ind = 0

    quad_pfmpc_plot_df = quad_pfmpc_dfs[quad_pfmpc_plot_ind]
    nn_plot_df = nn_dfs[nn_plot_ind]
    lfbk_plot_df = lfbk_dfs[lfbk_plot_ind]

    plot_nn_trajectory(nn_plot_df)
    # plot_quad_nn_lfbk(quad_pfmpc_plot_df, nn_plot_df, lfbk_plot_df)
    plot_mean_std_rms_error(
        [quad_pfmpc_dfs, nn_dfs, lfbk_dfs],
        method_names=["DMPC", "Neural network", "Linear feedback"],
    )
