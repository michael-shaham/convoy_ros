...
save_file_exp = prefix + '/convoy_ros/data_analysis/figures/iser_platoon_exp.pdf'
save_file_rms = prefix + '/convoy_ros/data_analysis/figures/iser_rms_fig.pdf'
...
def plot_quad_l1_lfbk(quad_df, l1_df, lfbk_df):
    ...
    fig.subplots_adjust(.1, .15, .89, .83, .07, .09)
    fig.suptitle(f'Platoon trajectories: Hardware comparison with {max(veh_inds) + 1} vehicles', 
                 fontsize=title_size)
    ...
    for i in veh_inds:
        ...
        ax[1,0].plot(quad_pfmpc_veh['time'].to_numpy(), 
                     quad_pfmpc_veh['ego_speed'].to_numpy(), 
                     label=f'{i}', zorder=max(veh_inds)-i,
                     color="C{}".format(i),
                     linewidth=linewidth, linestyle=linestyle, alpha=alphas[i])
        ax[1,1].plot(l1_pfmpc_veh['time'].to_numpy(), 
                     l1_pfmpc_veh['ego_speed'].to_numpy(), 
                     label=f'{i}', zorder=max(veh_inds)-i,
                     color="C{}".format(i),
                     linewidth=linewidth, linestyle=linestyle, alpha=alphas[i])
        ax[1,2].plot(lfbk_veh['time'].to_numpy(), 
    ...
    # ax[0,2].legend(bbox_to_anchor=(0.99, 0.5), loc="center left", 
    #                title="vehicle", title_fontsize=legend_title_size,
    #                fontsize=legend_font_size)
    ax[1,2].legend(bbox_to_anchor=(0.99, 1.1), loc="center left", 
                   title="vehicle", title_fontsize=legend_title_size,
                   fontsize=legend_font_size)
    plt.show()
...
def get_rms_errors(dfs: List[pd.DataFrame]):
    ...
    dist_rms_stds = np.array([np.std(dist_rms_errors[i], ddof=1) for i in range(n_vehicles)])
    speed_rms_means = np.array([np.mean(speed_rms_errors[i]) for i in range(n_vehicles)])
    speed_rms_stds = np.array([np.std(speed_rms_errors[i], ddof=1) for i in range(n_vehicles)])

    return (dist_rms_errors, speed_rms_errors, 
            dist_rms_means, dist_rms_stds, 
            speed_rms_means, speed_rms_stds)


def plot_mean_std_rms_error(df_list_of_lists: List[List[pd.DataFrame]], method_names: List[str]):
    assert len(df_list_of_lists) == len(method_names)

    fig, ax = plt.subplots(1, 2, sharey='row', figsize=(4.7, 1.75))
    fig.subplots_adjust(.09, .23, .77, .79, .08, .1)
    ...
    for i, method in enumerate(method_names):
        ...
        ax[0].fill_between(veh_inds, dist_rms_means - dist_rms_stds/np.sqrt(10),
                           dist_rms_means + dist_rms_stds/np.sqrt(10), alpha=0.4)
        ax[1].plot(veh_inds, speed_rms_means, color="C{}".format(i), label=method)
        ax[1].fill_between(veh_inds, speed_rms_means - speed_rms_stds/np.sqrt(10),
                           speed_rms_means + speed_rms_stds/np.sqrt(10), alpha=0.4)
...
if __name__ == '__main__':
    ...
    data_files.sort()
    lfbk_files = [file for file in data_files if 'iser_lfbk' in file]
    quad_pfmpc_files = [file for file in data_files if 'iser_quad_pfmpc' in file]
    l1_pfmpc_files = [file for file in data_files if 'iser_l1_pfmpc' in file]
    ...
    plot_mean_std_rms_error(
        ...
        method_names=[rf'$\|\|\cdot\|\|_2^2$ DMPC', '$\|\|\cdot\|\|_1$ DMPC', 'Linear feedback']
    )