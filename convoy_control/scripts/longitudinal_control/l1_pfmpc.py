...
""" @file l1_pfmpc.py
    ...
    predecessor following variant, to follow preceding vehicles, but uses 
    the l1 norm in the cost function.
"""
import numpy as np
...
from convoy_interfaces.msg import (DesiredDistance, LongitudinalControl, 
                                   LongitudinalStateTrajectory, NeighborInfo, 
                                   ConvoyControlData, TrainResetPositions)
from nav_msgs.msg import Odometry

from convoy_control.utils.longitudinal_state import (
    create_long_state_trajectory, create_long_state_np
)
from convoy_control.dynamics.linear_long_accel import LinearLongAccel
from convoy_control.mpc.sparse_l1_pfmpc import SparseL1PFMPC
from convoy_control.dynamics.linear_long_vel import LinearLongVel
from convoy_control.mpc.sparse_l1_pfmpc_vel import SparseL1PFMPCVel


class L1PFMPCNode(Node):

    def __init__(self):
        super().__init__('l1_pfmpc_node')

        # ROS parameters
        self.use_sim = self.declare_parameter(
            'use_sim').get_parameter_value().bool_value
        self.veh_ns = self.declare_parameter(
            'veh_ns').get_parameter_value().string_value
        veh_frame = self.declare_parameter(
            'veh_frame').get_parameter_value().string_value
        long_ctrl_topic = self.declare_parameter(
            'longitudinal_control_topic').get_parameter_value().string_value
        odom_topic = self.declare_parameter(
            'odom_topic').get_parameter_value().string_value
        neighbor_info_topic = self.declare_parameter(
            'neighbor_info_topic').get_parameter_value().string_value
        d_des_topic = self.declare_parameter(
            'desired_distance_topic').get_parameter_value().string_value
        control_data_topic = self.declare_parameter(
            'convoy_control_data_topic').get_parameter_value().string_value
        reset_topic = self.declare_parameter(
            'reset_topic').get_parameter_value().string_value
        shared_long_traj_topic = self.declare_parameter(
            'shared_long_traj_topic').get_parameter_value().string_value
        vl_traj_topic = self.declare_parameter(
            'virtual_leader_traj_topic').get_parameter_value().string_value
        # DMPC params
        self.neighbor_ind = self.declare_parameter(
            'neighbor_ind').get_parameter_value().integer_value
        self.d_des = self.declare_parameter(
            'desired_distance').get_parameter_value().double_value
        self.H = self.declare_parameter(
            'horizon').get_parameter_value().integer_value
        self.dt = self.declare_parameter(
            'dt').get_parameter_value().double_value
        self.tau = self.declare_parameter(
            'tau').get_parameter_value().double_value
        v_min = self.declare_parameter(
            'speed_min').get_parameter_value().double_value
        v_max = self.declare_parameter(
            'speed_max').get_parameter_value().double_value
        a_min = self.declare_parameter(
            'accel_min').get_parameter_value().double_value
        a_max = self.declare_parameter(
            'accel_max').get_parameter_value().double_value
        q_self_l1 = self.declare_parameter(
            'q_self_l1').get_parameter_value().double_value
        q_pred_l1 = self.declare_parameter(
            'q_pred_l1').get_parameter_value().double_value
        r_l1 = self.declare_parameter(
            'r_l1').get_parameter_value().double_value
        w = self.declare_parameter(
            'W_l1').get_parameter_value().double_array_value
        W = np.diag(w)
        ...
        self.neighbor_speed = None

        neighbor_long_traj_topic = "/veh_" + str(self.neighbor_ind) + "/" + \
        ...
        self.create_subscription(LongitudinalStateTrajectory, 
                                 neighbor_long_traj_topic, 
                                 self.neighbor_traj_cb, 1)
        ...
        if self.use_sim:
            ...
            self.pfmpc = SparseL1PFMPC(W, q_self_l1, q_pred_l1, r_l1, 
                                       self.dyn.A, self.dyn.B, self.dyn.C,
                                       a_min, a_max, self.H)
        else:
            self.dyn = LinearLongVel(self.dt, self.tau)
            self.pfmpc = SparseL1PFMPCVel(W, q_self_l1, q_pred_l1, r_l1, 
                                          self.dyn.A, self.dyn.B, 
                                          v_min, v_max, a_max, self.dt, self.H)
    ...
    def neighbor_traj_cb(self, msg: LongitudinalStateTrajectory):
        ...
        x_0 = np.array([0., self.curr_speed, self.curr_accel])
        ...
        if prob.status != 'optimal':
            if self.veh_ns == 'veh_0':
                self.get_logger().info(f"problem returned {prob.status} " + 
                                       f"for vehicle 0")
            else:
                self.get_logger().info(f"problem returned {prob.status} " + 
