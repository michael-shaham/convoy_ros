...
from convoy_interfaces.msg import (LongitudinalStateTrajectory, 
                                   TrainResetPositions)
...
from convoy_control.utils.step_reference import (accel_decel_step, accel_step, 
                                                 brake_step)
...
class VirtualLeader(Node):
    ...
    def __init__(self):
        ...
        self.vl_frame = self.declare_parameter(
            'virtual_leader_frame').get_parameter_value().string_value
        ...
        self.stop_time = self.declare_parameter(
            'stop_time').get_parameter_value().double_value
        self.wait_time = self.declare_parameter(
            'wait_time').get_parameter_value().double_value
        self.brake_decel = self.declare_parameter(
            'brake_decel').get_parameter_value().double_value
        self.traj_type = self.declare_parameter(
            'leader_trajectory_type').get_parameter_value().string_value
        self.zero_pct = self.declare_parameter(
            'zero_speed_percentage').get_parameter_value().double_value
        self.stag_T_min = self.declare_parameter(
            'stagger_time_min').get_parameter_value().double_value
        self.stag_T_max = self.declare_parameter(
            'stagger_time_max').get_parameter_value().double_value
        self.low_v_min = self.declare_parameter(
            'low_speed_min').get_parameter_value().double_value
        self.low_v_max = self.declare_parameter(
            'low_speed_max').get_parameter_value().double_value
        self.high_v_min = self.declare_parameter(
            'high_speed_min').get_parameter_value().double_value
        self.high_v_max = self.declare_parameter(
            'high_speed_max').get_parameter_value().double_value
        self.ref_accel_min = self.declare_parameter(
            'reference_accel_min').get_parameter_value().double_value
        self.ref_accel_max = self.declare_parameter(
            'reference_accel_max').get_parameter_value().double_value
        self.sin_T_min = self.declare_parameter(
            'sinusoid_period_min').get_parameter_value().double_value
        self.sin_T_max = self.declare_parameter(
            'sinusoid_period_max').get_parameter_value().double_value

        # ROS publishers
        self.vl_pub = self.create_publisher(LongitudinalStateTrajectory, 
                                            vl_traj_topic, 1)
        ...
        if not self.start_moving:
            ...
            if (len(states) < self.H+1):
                if curr_time - self.start_time > self.stop_time:
                    ...
                    x_ref, _ = brake_step(self.brake_decel, self.dt, self.stag_T_min, 
    ...
    def generate_trajectory(self):
        ...
        if self.traj_type == 'random':
            ...
            if traj_type == 0:
                ...
                x_ref_1, _ = accel_step(v_low, init_accel_time, self.dt, 
                                        init_accel_time + 1., self.curr_speed)
        ...
        if traj_type == 0:  # staggered velocity
            ...
            x_ref_2, _= accel_decel_step(
                stag_T, self.dt, repeat_accel_start, repeat_accel_end, 
