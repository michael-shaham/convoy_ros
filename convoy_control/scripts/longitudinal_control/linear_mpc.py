...
from convoy_interfaces.msg import (LongitudinalControl, 
...
class LinearMPCNode(Node):
    ...
    def __init__(self):
        ...
        self.veh_ns = self.declare_parameter('veh_ns').get_parameter_value().string_value
        veh_frame = self.declare_parameter('veh_frame').get_parameter_value().string_value
        long_ctrl_topic = self.declare_parameter('longitudinal_control_topic').get_parameter_value().string_value
        odom_topic = self.declare_parameter('odom_topic').get_parameter_value().string_value
        control_data_topic = self.declare_parameter('convoy_control_data_topic').get_parameter_value().string_value
        reset_topic = self.declare_parameter('reset_topic').get_parameter_value().string_value
        shared_long_traj_topic = self.declare_parameter('shared_long_traj_topic').get_parameter_value().string_value
        vl_traj_topic = self.declare_parameter('virtual_leader_traj_topic').get_parameter_value().string_value
        # MPC parameters
        self.neighbor_ind = self.declare_parameter('neighbor_ind').get_parameter_value().integer_value
        ...
        if self.n == 3:
            ...
            u_min = v_min
            u_max = v_max
        self.mpc = SparseLinearMPC(Q, Q_f, R, self.dyn.A, self.dyn.B, 
                                   u_min, u_max, x_min, x_max, self.H)
