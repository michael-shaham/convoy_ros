...
class ConstantInputNode(Node):
    ...
    def __init__(self):
        ...
        data_topic = self.declare_parameter(
            'convoy_control_data_topic').get_parameter_value().string_value
        self.speed_des = self.declare_parameter(
            'constant_speed').get_parameter_value().double_value
        self.accel_des = self.declare_parameter(
            'constant_accel').get_parameter_value().double_value
        ...
        self.long_pub = self.create_publisher(LongitudinalControl,
                                              long_ctrl_topic, 1)
        self.long_pub_timer = self.create_timer(.02, self.long_pub_cb)
