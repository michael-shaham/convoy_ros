...
from convoy_interfaces.msg import (DesiredDistance, LongitudinalControl, 
                                   ConvoyControlData, NeighborInfo, 
                                   TrainResetPositions)
...
class LinearFeedbackNode(Node):
    ...
    def __init__(self):
        ...
        self.speed_min = self.declare_parameter(
            "speed_min").get_parameter_value().double_value
        self.speed_max = self.declare_parameter(
            "speed_max").get_parameter_value().double_value
        self.accel_min = self.declare_parameter(
            "accel_min").get_parameter_value().double_value
        self.accel_max = self.declare_parameter(
        ...
        k_p_a = self.declare_parameter(
            'relative_position_gain_accel').get_parameter_value().double_value
        k_s_a = self.declare_parameter(
            'relative_speed_gain_accel').get_parameter_value().double_value
        k_p_v = self.declare_parameter(
            'relative_position_gain_vel').get_parameter_value().double_value
        k_s_v = self.declare_parameter(
            'relative_speed_gain_vel').get_parameter_value().double_value
    ...
    def reset_cb(self, msg: TrainResetPositions):
        ...
        long_control_msg.speed = 0.0
        long_control_msg.acceleration = 0.0
    ...
    def neighbor_info_cb(self, msg: NeighborInfo):
        ...
        input = self.long_controller.control(self.distance_error, 
                                             self.speed_error)
        if self.use_sim:
            des_speed = 0.0
            des_accel = input
