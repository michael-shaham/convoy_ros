...
class SparseQuadPFMPC:
    ...
    Parameters:
        ...
        H: horizon
    """
    def __init__(self, Q: np.ndarray, Q_p: np.ndarray, R: np.ndarray, 
                 A: np.ndarray, B: np.ndarray, C: np.ndarray, 
                 u_min: float, u_max: float, H: int):
    ...
    def control(self, x_0, y_a, y_p_a, d_des):
        ...
        for t, i in enumerate(range(self.m+self.p, len(z_opt), 
                                    self.n+self.m+3*self.p)):
            x_opt[:, t+1] = z_opt[i:i+self.n]
            u_opt[:, t+1] = z_opt[i+self.n:i+self.n+self.m]
            y_opt[:, t+1] = z_opt[i+self.n+self.m:i+self.n+self.m+self.p]
        x_opt[:, self.H] = self.Ad @ x_opt[:, self.H-1] + self.Bd @ u_opt[:, self.H-1]
