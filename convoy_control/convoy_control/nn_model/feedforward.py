...
class FullyConnected(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            the output tensor
        """
        if self.training and self.max_norm_val > 0:
            self.max_norm(self.max_norm_val)
    ...
    def max_norm(self, max_val=2.0):
        for layer in self.layers.children():
            if isinstance(layer, nn.Linear):
                weight_norm = layer.weight.data.norm(2, dim=0, keepdim=True).clamp(
                    min=max_val / 2
                )
                desired = torch.clamp(weight_norm, max=max_val)
                layer.weight.data *= desired / weight_norm
