...
""" @file course_walls_seg.py
    authors: Michael Shaham, Lisa Byrne
...
class WallSegNode(Node):
    ...
    def scan_cb(self, msg):
        ...
        pc = np.c_[proc_ranges * np.cos(proc_angles),
                   proc_ranges * np.sin(proc_angles)]
        ...
        clusters, _ = lidar_proc.cluster_pc_dbscan(pc, self.dbscan_eps,
                                                   self.dbscan_min_samples)
        # TODO: use outliers smartly (likely something to work on down the line)
        ...
        if len(clusters) == 0:
            ...
            # if the average angle > 0, then it's to the left, else left
            average_ang = np.mean(np.arctan2(cluster[:, 1], cluster[:, 0]))
            if average_ang >= 0:
            ...
            cluster_lens = np.array([len(cluster) for cluster in clusters])
            ind_1 = np.argmax(cluster_lens)
            cluster_lens[ind_1] = 0
            ind_2 = np.argmax(cluster_lens)

        avg_ang_1 = np.mean(np.arctan2(clusters[ind_1][:, 1], 
                                       clusters[ind_1][:, 0]))
        avg_ang_2 = np.mean(np.arctan2(clusters[ind_2][:, 1], 
                                       clusters[ind_2][:, 0]))
        # we will just assume the average angles are opposite
        if avg_ang_1 >= 0 and avg_ang_2 <= 0:
            left_pc = clusters[ind_1]
            right_pc = clusters[ind_2]
        elif avg_ang_1 <= 0 and avg_ang_2 >= 0:
            left_pc = clusters[ind_2]
            right_pc = clusters[ind_1]
        else:
            if avg_ang_1 >= 0:
                left_pc = clusters[ind_1]
                ...
                right_pc = clusters[ind_1]
                right_pc = np.c_[right_pc, np.zeros(right_pc.shape[0])]
        ...
        course_data.left_wall_dist = left_min_dist
        course_data.right_wall_dist = right_min_dist
        self.left_pc_pub.publish(course_data.left_wall)
        self.right_pc_pub.publish(course_data.right_wall)
        self.course_data_pub.publish(course_data)
