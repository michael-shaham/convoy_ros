...
class NeighborLocalizationSim(Node):
    ...
    def __init__(self):
        ...
        self.info_timer = self.create_timer(neighbor_timer_period,
                                            self.neighbor_pub_cb)
    ...
    def neighbor_pub_cb(self):
        ...
        ego_yaw = quat2euler([self.ego_pose.orientation.w,
                              self.ego_pose.orientation.x,
                              self.ego_pose.orientation.y,
                              self.ego_pose.orientation.z])[2]
        prec_yaw = quat2euler([self.neighbor_pose.orientation.w, 
                               self.neighbor_pose.orientation.x,
                               self.neighbor_pose.orientation.y,
                               self.neighbor_pose.orientation.z])[2]
        relative_yaw = prec_yaw - ego_yaw
        relative_quat = euler2quat(0., 0., relative_yaw)
    ...
    def ego_odom_cb(self, msg):
        if not self.ego_updated:
            ...
            ego_heading = quat2euler([msg.pose.pose.orientation.w,
                                    msg.pose.pose.orientation.x,
                                    msg.pose.pose.orientation.y,
                                    msg.pose.pose.orientation.z])[2]
            self.ego_R = np.array([[np.cos(ego_heading), np.sin(ego_heading)],
    ...
    def prec_odom_cb(self, msg):
        if not self.neighbor_updated:
            ...
            # neighbor speed
            self.prec_speed = np.sqrt(msg.twist.twist.linear.x**2 + 
