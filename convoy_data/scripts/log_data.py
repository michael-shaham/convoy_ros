...
class LogData(Node):
    def __init__(self):
        ...
        # ROS parameters
        joy_topic = self.declare_parameter(
            'joy_topic').get_parameter_value().string_value
        self.safety_button_ind = self.declare_parameter(
            'safety_button_ind').get_parameter_value().integer_value
        ...
        n_veh = self.declare_parameter(
            'n_vehicles').get_parameter_value().integer_value
        self.trial_name = self.declare_parameter(
            'trial_name').get_parameter_value().string_value
        self.samples = self.declare_parameter(
            'learn_samples_cut').get_parameter_value().integer_value
        
        self.column_names = ['veh_ind', 'time', 'des_dist', 'pred_dist', 'dist_err', 
                             'ego_speed', 'pred_speed', 'speed_err', 'opt_input']
        ...
        for i in range (n_veh):
            ...
            self.data_subs.append(self.create_subscription(
                ConvoyControlData, 'veh_' + str(i) + '/' + control_data_topic, 
                cb, 10))
    ...
    def save_cut(self):
        ...
        path_to_save = '~/cvy_ws/src/convoy_ros/convoy_data/data/' + cut_trial_name + '.csv'
