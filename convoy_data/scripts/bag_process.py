...
class BagProcess(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(AckermannDriveStamped, 'drive',  
                                 self.drive_cb, 100)
        self.create_subscription(NeighborInfo, 'neighbor_info', 
                                 self.neighbor_info_cb, 100)
        self.create_subscription(Odometry, 'odom', self.odom_cb, 100)
        self.create_subscription(PozyxRangeArray, 'pozyx/ranges', 
                                 self.pozyx_cb, 100)
    ...
    def drive_cb(self, msg):
        self.drive_times.append(msg.header.stamp.sec + 
    ...
    def neighbor_info_cb(self, msg):
        self.neighbor_times.append(msg.header.stamp.sec + 
                               msg.header.stamp.nanosec / 1e9)
        self.neighbor_dists.append(msg.distance)
        self.neighbor_speeds.append(msg.speed)
    
    def odom_cb(self, msg):
        self.speed_times.append(msg.header.stamp.sec + 
                                msg.header.stamp.nanosec / 1e9)
        self.speeds.append(np.linalg.norm(np.array([msg.twist.twist.linear.x,
                                                    msg.twist.twist.linear.y,
                                                    msg.twist.twist.linear.z])))

    def pozyx_cb(self, msg):
        self.range_times.append(msg.header.stamp.sec + 
                                msg.header.stamp.nanosec / 1e9)
        self.ranges.append(msg.pozyx_ranges[0].distance)

    def on_shutdown(self):
        save_location = os.path.join(os.path.expanduser('~'), 'cvy_ws', 'src', 
                                     'convoy_ros', 'data_analysis', 'data',
                                     self.test_name)
        print("Saving bag data.")

        drive_data_file = os.path.join(save_location, 
                                       f'{self.veh_ns}_drive_data.csv')
        drive_data = np.array([self.drive_times, 
                               self.drive_speeds, 
                               self.drive_accels,
                               self.drive_steers])
        np.savetxt(drive_data_file, drive_data)

        neighbor_data_file = os.path.join(save_location, 
                                          f'{self.veh_ns}_neighbor_data.csv')
        neighbor_data = np.array([self.neighbor_times, 
                                  self.neighbor_dists, 
                                  self.neighbor_speeds])
        np.savetxt(neighbor_data_file, neighbor_data)

        speed_data_file = os.path.join(save_location, 
                                       f'{self.veh_ns}_speed_data.csv')
        speed_data = np.array([self.speed_times, self.speeds])
        np.savetxt(speed_data_file, speed_data)

        range_data_file = os.path.join(save_location,
                                       f'{self.veh_ns}_range_data.csv')
        range_data = np.array([self.range_times, self.ranges])
        np.savetxt(range_data_file, range_data)
