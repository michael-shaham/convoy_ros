...
def generate_launch_description():
    ...
    head_long_controller_la = DeclareLaunchArgument(
        'head_long_controller', default_value='quad_pfmpc',
        description='The controller that will be used to select speeds/accels',
        choices=['bicycle_mpc', 'constant_input', 'double_int_mpc', 
                 'quad_pfmpc', 'l1_pfmpc', 'linear_mpc', 'track_vl']
    )
    ...
    head_lat_controller_la = DeclareLaunchArgument(
        'head_lat_controller', default_value='pure_pursuit',
    ...
    trail_long_controller_la = DeclareLaunchArgument(
        'trail_long_controller', default_value='quad_pfmpc',
    ...
    trail_lat_controller_la = DeclareLaunchArgument(
        'trail_lat_controller', default_value='pure_pursuit',
        description='Controller used to determine acceleration of trail cars.',
    ...
    head_include = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([
                FindPackageShare('convoy_control'), 'launch',
    ...
    data_vis_la = DeclareLaunchArgument(
        'data_vis', default_value='false',
        choices=['true', 'false']
    )
    ld.add_action(data_vis_la)

    plotjuggler_layout = os.path.join(
        get_package_share_directory('convoy_data'), 'plotjuggler', 
    ...
    trial_name_la = DeclareLaunchArgument(
        'trial_name', default_value='test'
    )
    ...
    log_data_include = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([
                FindPackageShare('convoy_data'), 'launch',
                'log_data.launch.py'
            ])
        ),
        launch_arguments={
    ...
    model_name_la = DeclareLaunchArgument('model_name', default_value='',
        description='Neural network model name for running the neural network.')
